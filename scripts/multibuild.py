...
@command(admin_only=False)
def mbreg(connection):
    """
    Register your starting blocks (required before /mb)
    /mbreg
    """
...
def mbmirror(connection, mirror=0):
    """
    Mirror build (mind your orientation during /mbreg)
    /mbmirror <1 2>
    """
    mirror = int(mirror)
...
def mbshape(connection, shape=None, adjust_z=0):
    """
    Load a prefab shape instead of using /mbreg
    /mbshape <ball box diamond land pyramid>
    """
...
def mbground(connection, level=None):
    """
    Don't build or destroy on ground level during /mb
    /mbground
    """
    by_user = False
...
def rollout_multiblocks(self, coord, destroy=False):
    ...
    for regblock in reversed(self.regblocks):
        ...
        if is_invalid_coord(mb_x, mb_y, mb_z) or mb_z >= self.protect_ground or self.on_block_build_attempt(mb_x, mb_y, mb_z) == False:
            continue
